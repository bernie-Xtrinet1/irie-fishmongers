...
function OrdersSummaryCardImpl(): React.ReactElement {
  const query = useDashboardSummary((data) => data.orders);
      ...
      {(orders) => (
        <div className="flex flex-col gap-4">
          <div>
            ...
            <p className="text-2xl font-semibold text-gray-900">{orders.customerOrdersTotal}</p>
          </div>
          <div>
            ...
            <dl className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
              {Object.entries(orders.vendorOrdersByStatus).map(([status, count]) => (
