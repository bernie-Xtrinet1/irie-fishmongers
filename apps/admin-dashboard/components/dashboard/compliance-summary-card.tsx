...
function ComplianceSummaryCardImpl(): React.ReactElement {
  const query = useDashboardSummary({ widget: 'compliance', staleTimeMs: 10_000, refetchIntervalMs: 10_000 });
      ...
      {(data) => (
        <div className="flex flex-col gap-4">
          <div>
            ...
            <p className="text-2xl font-semibold text-gray-900">{data.compliance.activeRecalls}</p>
          </div>
          <div>
            <p className="text-xs font-medium uppercase tracking-wide text-gray-500">Active alerts by severity</p>
            <dl className="mt-2 flex flex-col gap-1 text-sm">
              {Object.entries(data.compliance.activeAlertsBySeverity).map(([severity, count]) => (
